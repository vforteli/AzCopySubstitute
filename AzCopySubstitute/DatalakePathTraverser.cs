...
internal class DataLakePathTraverser
{
    ...
    /// <param name="cancellationToken"></param>
    /// <returns>Task which completes when all items have been added to the blocking collection</returns>
    public Task ListPathsNoRecursiveAsync(DataLakeFileSystemClient dataLakeFileSystemClient, string searchPath, BlockingCollection<string> paths, int maxThreads = 16, CancellationToken cancellationToken = default) => Task.Run(async () =>
    {
        var filesCount = 0;
        var directoryPaths = new BlockingCollection<string>();
        var tasks = new ConcurrentDictionary<Guid, Task>();
        var sw = Stopwatch.StartNew();
